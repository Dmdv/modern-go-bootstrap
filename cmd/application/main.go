...
import (
	...
	"github.com/dmdv/modern-go-bootstrap/internal/app/sample"
	"github.com/dmdv/modern-go-bootstrap/internal/app/sample/todo/tododriver"
	"github.com/dmdv/modern-go-bootstrap/internal/common/commonadapter"
	"github.com/dmdv/modern-go-bootstrap/internal/platform/appkit"
	"github.com/dmdv/modern-go-bootstrap/internal/platform/database"
	"github.com/dmdv/modern-go-bootstrap/internal/platform/gosundheit"
	"github.com/dmdv/modern-go-bootstrap/internal/platform/log"
	"github.com/dmdv/modern-go-bootstrap/internal/platform/watermill"
)
...
func main() {
	...
	// Set up app server
	{
		...
		// In larger apps, this should be split up into smaller functions
		{
			...
			sample.InitializeApp(httpRouter, grpcServer, publisher, config.App.Storage, db, logger, errorHandler)

			h, err := watermill.NewRouter(logger)
			emperror.Panic(err)

			err = sample.RegisterEventHandlers(h, subscriber, logger)
			emperror.Panic(err)
